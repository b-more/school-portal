            ...
            ->schema([
                    ...
                    ->options(function () {
                        return FeeStructure::query()
                            ...
                            ->mapWithKeys(function ($feeStructure) {
                                // Create a descriptive label with grade, term and year
                                $label = "{$feeStructure->grade} - {$feeStructure->term} - {$feeStructure->academic_year} (ZMW {$feeStructure->total_fee})";
                                return [$feeStructure->id => $label];
                            });
                    })
                    ->searchable()
                    ->required()
                ...
                Forms\Components\TextInput::make('amount_paid')
                    ...
                    ->afterStateUpdated(function ($state, callable $set, $get) {
                        if ($get('fee_structure_id')) {
                            $feeStructure = FeeStructure::find($get('fee_structure_id'));
                            if ($feeStructure) {
        ...
        return $table
            ->columns([
                Tables\Columns\TextColumn::make('student.name')
                    ->searchable()
                    ->sortable(),
                Tables\Columns\TextColumn::make('feeStructure.grade')
                    ->label('Grade')
                    ->sortable(),
            ...
            ->filters([
                ...
                Tables\Filters\SelectFilter::make('grade')
                    ->label('Grade')
                    ->options(function() {
                        return FeeStructure::distinct('grade')->pluck('grade', 'grade')->toArray();
                    }),
                Tables\Filters\SelectFilter::make('term')
                    ->label('Term')
                    ->options(function() {
                        return FeeStructure::distinct('term')->pluck('term', 'term')->toArray();
                    }),
                Tables\Filters\SelectFilter::make('academic_year')
                    ->label('Academic Year')
                    ->options(function() {
                        return FeeStructure::distinct('academic_year')->pluck('academic_year', 'academic_year')->toArray();
                    }),
