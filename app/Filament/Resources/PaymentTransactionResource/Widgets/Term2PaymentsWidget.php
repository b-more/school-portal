...
class Term2PaymentsWidget extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        $term2 = Term::where('name', 'Term 2')
            ->orWhere('name', 'Second Term')
            ->first();

        if (!$term2 || !$currentYear) {
            return [
                Stat::make('Term 2 Payments', 'No Data')
                    ->description('Term 2 or Academic Year not found')
                    ->descriptionIcon('heroicon-m-exclamation-triangle')
                    ->color('warning'),
            ];
        }

        $query = PaymentTransaction::whereHas('studentFee', function ($q) use ($student, $term2, $currentYear) {
            $q->where('student_id', $student->id)
              ->whereHas('feeStructure', function ($fq) use ($term2, $currentYear) {
                  $fq->where('term_id', $term2->id)
        ...
        // Get expected fee for this student in Term 2
        $studentFee = \App\Models\StudentFee::where('student_id', $student->id)
            ->whereHas('feeStructure', function ($q) use ($term2, $currentYear) {
                $q->where('term_id', $term2->id)
                  ->where('academic_year_id', $currentYear->id);
            })
            ->first();
        ...
        return [
            Stat::make('Term 2 - ' . $student->grade->name, 'ZMW ' . number_format($totalAmount, 2))
